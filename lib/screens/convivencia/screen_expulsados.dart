...
Widget build(BuildContext context) {
    ...
    body: Consumer<ConvivenciaProvider>(
      builder: (context, convivenciaProvider, child) {
        ...
        if (filteredExpulsados.isEmpty && _controller.text.isEmpty) {
          // Filtrar objetos donde no hay valores vacíos
          filteredExpulsados = listadoExpulsados.where((expulsado) {
            return expulsado.idAlumno.isNotEmpty &&
                   expulsado.fecInic.isNotEmpty &&
                   expulsado.fecFin.isNotEmpty;
          }).toList();
        }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (expulsado.curso.isNotEmpty)
                            Text('Curso: ${expulsado.curso}', style: const TextStyle(fontSize: 14)),
                          if (expulsado.tipoExpulsion.isNotEmpty)
                            Text('Tipo Expulsión: ${expulsado.tipoExpulsion}', style: const TextStyle(fontSize: 14)),
                          if (expulsado.observaciones != null && expulsado.observaciones!.isNotEmpty)
                            Text('Observaciones: ${expulsado.observaciones}', style: const TextStyle(fontSize: 14)),
                          const SizedBox(height: 5),
                          ...
                          Row(
                            ...
                            children: [
                              if (expulsado.fecInic.isNotEmpty && expulsado.fecFin.isNotEmpty)
                                Text('Inicio: ${expulsado.fecInic}', style: const TextStyle(fontSize: 14)),
                              if (expulsado.fecFin.isNotEmpty)
                                Text('Final: ${expulsado.fecFin}', style: const TextStyle(fontSize: 14)),
                            ],
                          ),
                          if (expulsado.fecVuelta.isNotEmpty)
                            Text('Vuelta: ${expulsado.fecVuelta}', style: const TextStyle(fontSize: 14)),
                        ],
                      ),
                    ),
                  );
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != selectedDate) {
      setState(() {
        selectedDate = picked;
        filterSearchResults(_controller.text); // Actualizamos el filtro con el texto actual
      });
    }
  }
  ...
  DateTime _parseDate(String date) {
    ...
    if (date.isEmpty) {
      return DateTime(1900); // Devolver una fecha predeterminada si la fecha está vacía
    }
